...
// As of Oct 2022, we are no longer using the register type modifier,
// as modern compilers now ignore it and darwin actually throws a warning.
#define IDL_REGISTER
...
#define IDL_VENDOR_NAME "L3Harris Geospatial Solutions, Inc."
...
#define IDL_VERSION_MINOR 9
#define IDL_VERSION_SUB 0
#define IDL_REVISION 452795
 ...
 */
#define IDL_LMGR_CLIENTSERVER           0x01   /* defunct, but preserved since formerly published */
#define IDL_LMGR_DEMO                   0x02   /* defunct, but preserved since formerly published */
#define IDL_LMGR_EMBEDDED               0x04   /* defunct, but preserved since formerly published */
#define IDL_LMGR_RUNTIME                0x08
#define IDL_LMGR_STUDENT                0x10   /* defunct, but preserved since formerly published */
#define IDL_LMGR_TRIAL                  0x20   /* defunct, but preserved since formerly published */
#define IDL_LMGR_CALLAPPNOCHECKOUT      0x40   /* defunct, but preserved since formerly published */
#define IDL_LMGR_CALLAPPLICINTERNAL		  0x80   /* defunct, but preserved since formerly published */
#define IDL_LMGR_VM                     0x100
...
#define IDL_INIT_BUFFER_LICENSE		(1 << 31)
                /* Use a buffer as a license source. If set the values in
                   IDL_INIT_DATA.bufferlicense (id, data, length) are used.
                   If those values are incorrect or not set the behavior
                   is undefined. */
...
typedef struct {
  ...
  /* Use of the bufferlicense structure requires the 
   * IDL_INIT_BUFFER_LICENSE option to be set
   */
  struct {
    const char *id;            /* Device ID of the buffer */
    const unsigned char *data; /* Contents of the buffer */
...
/*
 * Clang10 on Darwin has these functions, and including iostream and this file
 * causes a conflict.
 * No one else does yet.
 * So, use our version unless you opt into the system version.
 */
#ifndef SYSTEM_SUPPLIES_STRLCPY
#define IDL_SUPPLIES_STRLCPY
#endif

#ifndef SYSTEM_SUPPLIES_STRLCAT
...
extern IDL_VPTR IDL_CDECL IDL_FindNamedVariableLevel(char *name, int
        ienter, int level);
...
extern char *IDL_CDECL IDL_AppUserDirRootPath(void);
...
extern void IDL_CDECL IDL_HeapIncrRefCount(const IDL_HVID *hvid,
        IDL_MEMINT n);
extern void IDL_CDECL IDL_HeapDecrRefCount(const IDL_HVID *hvid,
        IDL_MEMINT n);
extern int IDL_CDECL IDL_ObjCallMethodByString(const char *methName,
        IDL_HVID obj, IDL_VPTR *result,  int argc, IDL_VPTR *argv, char *argk);
...
extern char *IDL_CDECL IDL_GetUserIDAsString( char *s, size_t n );
extern char *IDL_CDECL IDL_GetProcessIDAsString( char *s, size_t n );
...
extern void IDL_CDECL IDL_WidgetBrowser_Message(void* userData, int type,
        ...);
...
extern int IDL_CDECL IDL_Main(int argc, char *argv[]);
...
extern IDL_VPTR IDL_CDECL IDL_Variable_Diff(int argc, IDL_VPTR *argv, char
        *argk);
extern IDL_VPTR IDL_CDECL IDL_Variable_toString(int argc, IDL_VPTR *argv,
        char *argk);
